...
<html>
    ...
    <body>
        ...
        <form class="layui-form x-center" action="" style="width:500px">
            ...
            <xblock><button class="layui-btn layui-btn-danger" onclick="delAll()"><i class="layui-icon">&#xe640;</i>批量删除</button><button class="layui-btn" onclick="new_add('添加用户','back/new_add.jsp','600','500')"><i class="layui-icon">&#xe608;</i>添加</button><span class="x-right" style="line-height:25px">共有数据：88 条</span></xblock>
            <table class="layui-table">
                ...
                <c:if test="${not empty requestScope.list}">
                     <c:forEach var="news" items="${requestScope.list}" varStatus="status">
                	...
                	<tr>
                        ...
                        <td><c:out value="${news.modlue}"></c:out></td>
                           ...
                           <c:set var="num" value="${news.img}" scope="request"></c:set>
                    		...
                    		img=(String)request.getAttribute("num");
                   			...
                   			img=img.substring(img.lastIndexOf("\\")+1,img.length());
                   			 %>
                   			<img src="upload/<%=img %>" alt="无图" width="56" height="56">
                         </c:if>
                         <c:if test="${empty news.img}&&${news.img ==''}">
                        ...
                        <td class="td-manage">       
                            ...
                            <%
                      			if(img!=null){
                      			%>
                      			<a title="修改" href="javascript:;" onclick="new_edit('修改','back/new_edit.jsp?id=${news.id}&title=${news.title}&module=${news.modlue }&content=${news.content}&createTime=${news.createTime}&img=<%=img%>','4','','510')"
                                ...
                                <i class="layui-icon">&#xe642;</i>
                      			...
                      			%>
                      			<a title="修改" href="javascript:;" onclick="new_edit('修改','back/new_edit.jsp?id=${news.id}&title=${news.title}&module=${news.modlue }&content=${news.content}&createTime=${news.createTime}&img=null','4','','510')"
                           			 class="ml-5" style="text-decoration:none">
                            ...
                            <a title="删除" href="javascript:;"  onclick="banner_del(this,'${news.id}')" 
                            style="text-decoration:none">
                            <!-- href="NewsServlet?method=del&id=${news.id}" -->
                            <%-- javascript:click(${news.id}); --%>
                                <i class="layui-icon">&#xe640;</i>
                            </a>
                        </td>
                    </tr>
